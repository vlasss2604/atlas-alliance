...
describe("9/10/11. identity only — no memory, no project logic, no wiring", () => {
  it("9/10. the migration and schema name no project and no memory concept", async () => {
    ...
    const migration = await readFile(
      "src/server/db/migrations/0044_onchain_observation_identity.sql",
      "utf-8",
    );
    ...
    expect(migration).toContain(
      'CREATE UNIQUE INDEX IF NOT EXISTS "uq_onchain_artifacts_job_observation"',
    );
    expect(migration).toContain(
      'CREATE UNIQUE INDEX IF NOT EXISTS "uq_onchain_artifacts_standalone_observation"',
    );
