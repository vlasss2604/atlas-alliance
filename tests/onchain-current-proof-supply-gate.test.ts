...
describe("15/16/17/18. boundaries", () => {
  ...
  // PHASE 1 BOUNDARY. This assertion originally also fingerprinted Budget
  // Reservation V2 by name (`deterministicCeilingForComponent`,
  // `planDeterministicDemand`). That implementation is deliberately NOT part
  // of this recovery — it is reconciled separately against the current
  // source-open work — so naming its internals here would assert the presence
  // of code this phase does not bring. The invariant that actually matters is
  // unchanged and still checked: the source-open reserve knows nothing about
  // the post-event read or the current-proof gate.
  it("16. the source-open reserve is untouched by this recovery", async () => {
    const { readFile } = await import("node:fs/promises");
