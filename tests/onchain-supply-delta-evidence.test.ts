...
describe("20/22..26. boundaries", () => {
  ...
  it("the migration is additive: no rewrite, no delete, no merge", async () => {
    ...
    // IF NOT EXISTS is the recovery's idempotency guard, required so the
    // migration is safe to re-run; it does not weaken the schema.
    expect(sqlText).toContain("ADD VALUE IF NOT EXISTS 'TOTAL_SUPPLY_DELTA'");
