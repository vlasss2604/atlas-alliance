...
describe("fake interpreter — known assets", () => {
  ...
  it("recognizes Lido from the Founder's canonical unseen-validation wording", async () => {
    const r = await classify(
      "As of today, how does value generated by the Lido protocol flow through the DAO, and what evidence shows that any of it currently reaches LDO through an active economic mechanism?",
    );
    expect(r.status).toBe("READY");
    expect(r.route).toBe("DEEP_RESEARCH");
    expect(r.project_or_asset).toBe("Lido");
    expect(r.related_entities).toEqual([]);
  });
