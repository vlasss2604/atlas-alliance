...
//
// SOURCE_OF_VALUE IS NOT ONE OF THEM ANY MORE, and its absence is derived
// rather than typed here: it mapped to TOKEN_SUPPLY until that entry was
// removed from INTENTS_BY_COMPONENT, because a supply LEVEL cannot answer
// where value comes from. Two guaranteed anchor reads is what this contract
// now schedules, so two is what the reservation must protect — listing a
// third would over-reserve, taken straight out of documentary acquisition.
...
describe("the reservation is the contract's demand, not one flat number", () => {
  ...
  it("6. no reachable chain means no promotion reserve is held", () => {
    ...
    expect(r.documentaryCeiling).toBe(22);
  });
  ...
  it("the half-ceiling cap gives up chain depth before a guaranteed read", () => {
    ...
    expect(r.baseReserved).toBe(2);
    expect(r.promotionReserved).toBe(2);
...
describe("1/2/4. the PUMP-shaped worst case, saturated, against the real ledger", () => {
  it("documentary saturation still leaves every deterministic open possible", async () => {
...
describe("13/14/15. boundaries", () => {
  ...
  it("15. NET_EFFECT applicability and Evidence semantics are unchanged", async () => {
    ...
    // TOTAL_SUPPLY_DELTA EXISTS NOW, and a blanket string ban would only
    // re-assert that. What must still be true — and is the thing this guard
    // was protecting — is that the budget work grants it nothing: no
    // applicability entry, so nothing may read it across a component
    // boundary, and it is not a gross-reduction kind.
    expect(facts).not.toContain("TOTAL_SUPPLY_DELTA: [");
    expect(facts).toContain('GROSS_SUPPLY_REDUCTION_FACT_KINDS: readonly OnchainFactKind[] = ["BURN"]');
  });
...
describe("the work queue is what the reservation reads", () => {
  it("a real job's demand is derived from its own contract", async () => {
    ...
    // TWO guaranteed anchor reads, not three: SOURCE_OF_VALUE still admits
    // on-chain acquisition as a component, but no longer maps to an
