...
// THE CEILING AN OPPORTUNISTIC SPENDER MUST PASS.
//
// Capacity that NO component holds — what is genuinely spare once every
// protected allocation is set aside. A spender with no protected allocation
// of its own passes this and therefore cannot make a guaranteed anchor read
// or a reachable promotion chain unaffordable; it proceeds only on what
// nothing else needed, and refuses cleanly when only protected capacity
// remains.
//
// It is the same number documentary acquisition passes, and deliberately so:
// "holds no protection" is one idea, not two. It is named separately because
// a DETERMINISTIC read can also be unprotected — an opportunistic completion
// that no component's contract guarantees — and calling such a read's
// ceiling `documentaryCeiling` would misdescribe it. This module names no
// consumer; it answers how much capacity is spare, and callers decide
// whether that is the ceiling they are entitled to.
export function unprotectedCeiling(reserve: OnchainSourceOpenReserve): number {
  return Math.max(0, reserve.maxSourceOpens - reserve.reserved);
}
