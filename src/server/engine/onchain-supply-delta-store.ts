...
export async function persistTotalSupplyDeltaEvidence(
  ...
  const [inserted] = await db
    ...
    .values({
      ...
      // reach a verdict yet.
      //
      // PHASE 1 BOUNDARY. Turning this into a typed direction is what lets
      // reconciliation reach a NET_EFFECT verdict from a delta, and that —
      // together with the CONTRADICTED path it enables — is deliberately not
      // recovered here. The foundation observes, derives and files; it does
      // not decide.
