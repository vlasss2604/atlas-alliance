...
// DESTINATION-BOUND BURN — a structural association, derived and nothing more.
//
...
//    persisted derivation path, from an address a first-party document of
//    THIS job named as a destination of this mechanism?"
//
// WHAT IT DOES NOT ANSWER, AND THE NAME IS DELIBERATE. An earlier diagnostic
// proposed calling this ATTRIBUTED. That word was refused, and refusing it is
// the point of this module: structural reachability does not show that the
// burned tokens were acquired by the mechanism, that the mechanism caused the
// burn, or that anything funded it. `ONCHAIN_DOES_NOT_PROVE.BURN` already
// states that linking a burn to a buyback needs separate admitted evidence,
// and this module does not supply it. BOUND is an association between an
// account and a document. It is not a cause, and it is not an acquisition.
//
...
export interface DestinationBoundBurnInput {
  ...
  // Addresses a first-party document of THIS job named. `AdmittedLocator` is
  // the c502112 type: current-job scoped by construction, and carrying the
  // Evidence and source that admitted it. Imported as a TYPE only, so no
  // database module enters this one's runtime graph.
  admittedDestinations: readonly AdmittedLocator[];
}
...
export interface DestinationBoundBurn {
  chain: string;
  network: string;
  projectAnchor: string;
  ...
  // admitted destination — zero hops, which the promotion rules explicitly
  // allow when a documented address is itself a token account for the mint.
  path: readonly DerivationPathStep[];
  hops: number;
}

export type DestinationBoundBurnRefusal =
  ...
  // No admitted destination was supplied at all.
  | "ROOT_NOT_ADMITTED"
  // Traversal ran out of edges before reaching an admitted destination.
  | "NO_DERIVATION_PATH"
  ...
  // More than one distinct parent, or more than one admitted destination,
  // could stand at the same position. Never silently resolved.
  | "AMBIGUOUS_ROOT"
  // The path revisits a subject.
  | "CYCLIC_PATH"
  // Still not at an admitted destination after the bounded traversal.
  | "PATH_DEPTH_EXCEEDED";

export type DestinationBoundBurnOutcome =
  | { bound: true; result: DestinationBoundBurn }
  | { bound: false; reason: DestinationBoundBurnRefusal };
...
): DestinationBoundBurnRefusal | null {
...
export function deriveDestinationBoundBurn(
  input: DestinationBoundBurnInput,
  ...
  // Destinations of THIS job only. An AdmittedLocator carries its own job id
  // (c502112), and one that belongs to another job is not a destination this
  // burn could be bound to — it is a different run's conclusion.
  const destinations = new Map<string, AdmittedLocator[]>();
  for (const d of input.admittedDestinations) {
    if (d.researchJobId !== boundary.researchJobId) continue;
    const existing = destinations.get(d.value);
    if (existing) existing.push(d);
    else destinations.set(d.value, [d]);
  }
  if (destinations.size === 0) return { bound: false, reason: "ROOT_NOT_ADMITTED" };
  ...
  for (let hop = 0; hop <= MAX_DERIVATION_PATH_EDGES; hop++) {
    const admitted = destinations.get(current);
    ...
    if (admitted) {
      ...
      // address that is BOTH an admitted destination and itself derived from
...
export const DESTINATION_BOUND_BURN_DOES_NOT_PROVE =
  "This shows only that the token account a deterministic burn destroyed tokens from is reachable, " +
  "through this job's own recorded derivation steps, from an address a first-party document of this " +
  "job named as a destination. It does NOT establish that the burned tokens were purchased by any " +
  "buyback; it does NOT establish that protocol revenue funded them; it does NOT establish when or how " +
  "the tokens were acquired; it does NOT establish that the mechanism caused the burn; and it does NOT " +
  "establish net deflation, any change in total or circulating supply, or holder value accrual.";
