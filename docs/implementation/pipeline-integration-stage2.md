...
> **Статус: ПРИНЯТО И ЗАМОРОЖЕНО владельцем, 2026-08-23, на HEAD
> `8206a79`.** См. `pipeline-integration-stage2-freeze.md` — freeze-
> документ, авторитетный для замороженной границы и правила
> переоткрытия. Продолжает Stage 1 (D-113/D-114,
...
> closure pass, зафиксированном D-116 (см. §10 ниже). D-115's исходная
> формулировка про `target_ref` была точна для сырого текста исключения,
> но слишком широка для credential-подобных query-параметров самого URL
> — уточнено D-116 и `pipeline-integration-stage2-freeze.md` §7. Финальная
> приёмка (D-117) заморозила `8206a79` — `ACCEPT WITH NON-BLOCKING NOTES`
> (LOW 4 принятых). S7/S6/S5/S4
...
**Stage 2 ПРИНЯТ И ЗАМОРОЖЕН на `8206a79` (D-117).** См.
`pipeline-integration-stage2-freeze.md` для авторитетной замороженной
границы и правила переоткрытия.
