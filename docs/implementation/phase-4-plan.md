...
> Статус: **ЗАМОРОЖЕНО** — см. `phase-4-freeze.md` (Interpreter v1 принят
> владельцем после живого прогона). Этот документ остаётся историей решений.
>
...
## 9. Итог реализации и adversarial review

Реализация: коммиты `47be37c` (Фаза 4), `0635c88` (исправления по ревью).
Проверки: tsc, eslint, `next build`, 59/59 vitest (40 регресс Фаз 1–3),
7/7 e2e (390×844), миграции и повторный сид на чистой БД.

Независимое ревью со свежим контекстом дало 7 находок; 3 блокирующие
исправлены, 4 низкоприоритетные исправлены тем же кругом.

| # | Находка | Решение |
|---|---------|---------|
| HIGH-1 | Гейт проверял только основную сущность: DEMO получал сравнение с CORE-проектом, а задача молча усекалась до одного проекта | Принято. Схема получила `related_entities`; сервер резолвит все сущности; scope/entitlement решаются для задачи целиком; `normalized_task` несёт все слаги |
| MEDIUM-2 | При `research_enabled=false` (отгружаемая конфигурация) объяснение показывалось как заблокированный Proof, а «интернет на Луне» — с ответом и отказом одновременно | Принято. Маршрут проверяется раньше рубильника; на не-исследовательских маршрутах кнопки Proof нет вовсе |
| MEDIUM-3 | При неоднозначном названии ответ пользователя не доходил до модели — гарантированный тупик на 2 уточнения | Принято. Вопрос от сервера называет варианты; ответ передаётся модели всегда |
| LOW-4 | Превью могло сказать `research: AVAILABLE` там, где enforcement даёт 403; сохранённая строка имела статус и маршрут, несовместимые по схеме | Принято. `AVAILABLE` ⇔ запуск разрешён; статус и маршрут понижаются вместе |
| LOW-5 | Закрывающий тег внутри вопроса подделывал границу блока данных | Принято. Границы экранируются |
| LOW-6 | Двойное нажатие оплачивало два вызова модели до отказа БД | Принято. Дешёвая проверка ребёнка до вызова; барьером остаётся уникальный индекс |
| LOW-7 | Постоянные 409 уточнения показывались как временный сбой «попробуйте ещё раз» | Принято. Честные тексты, форма уточнения закрывается |

Отдельно исправлена честность тестов: тест сравнения больше не подставляет
сущность (gateway извлекает обе стороны сам), добавлено покрытие
отгружаемой конфигурации `research_enabled=false`, проверка выхода из
неоднозначности и защита инструкции «ввод — недоверенные данные» от тихого
удаления из системного промпта.

Дополнительно (по замечаниям владельца к продуктовому поведению):
- экран уточнения сначала показывает **что уже понято**, потом задаёт один
  короткий вопрос — «Atlas разобрался», а не «Atlas классифицировал»;
- быстрый ответ перестал быть тупиком: предлагает проверить вопрос на
  конкретном проекте (это новая интерпретация, гейт `DEEP_RESEARCH`
  не ослаблен).

**Не проверено:** качество понимания живой моделью. `tests/interpreter-live.test.ts`
(4 кейса, включая prompt-инъекцию) пропускается без `ANTHROPIC_API_KEY`.

---

## 10. Closed Smoke Test Интерпретатора (директива владельца)

`scripts/smoke-interpreter.ts` — 20 реальных человеческих вопросов
(разговорные, со сленгом, опечатками, кириллическим написанием проектов,
неправильной раскладкой, без названного проекта, сравнение, ошибка
категории, инвестсовет, вне области, бессмыслица, prompt-инъекция).

Проверяются четыре вещи, а не «красота ответа»:
1. понято ли намерение;
2. верно ли выбран маршрут;
3. хорош ли уточняющий вопрос;
4. не придумано ли лишнее.

Ожидаемый маршрут у каждого вопроса — ориентир для глаза: расхождение
помечается «?», но не объявляется провалом. Качество оценивает владелец.

```
Живой прогон:      ANTHROPIC_API_KEY=sk-ant-... npm run smoke:interpreter
Отчёт в файл:      SMOKE_OUT=smoke.md ANTHROPIC_API_KEY=... npm run smoke:interpreter
Проверка проводки: MODEL_GATEWAY=fake npm run smoke:interpreter
```

Прогон печатает по каждому вопросу маршрут, статус, intent и уверенность,
извлечённые сущности, research task, уточняющий вопрос, быстрый ответ,
допущения и неясности; в конце — число расхождений, сбоев и стоимость
прогона в токенах.

**Статус: выполнено.** Живой прогон состоялся (5 итераций по мере
починки: 11 сбоев → 0). Отчёт — `phase-4-live-smoke.md`, разбор —
`phase-4-freeze.md` §3.
