...
## 8. STRATEGY REVIEW REQUIRED — решения владельца до реализации

1. **§2.5 Вариант A или B** — показывать ли `quick_answer` (AI-текст без
   Proof) для QUICK_EXPLANATION / NO_RESEARCH_NEEDED. Рекомендую A
   (соответствует канону atlas-intent), с лимитом длины.
2. **§2.3 Модель** — подтвердить `claude-haiku-4-5` как дефолт
   (канон «дешёвая модель» против общего дефолта opus в skill claude-api;
   переключение — ключом конфига без деплоя).
3. **§3.3 `interpreter_enabled=true` с Фазы 4** — Ask-экран работает вживую
   (интерпретация + уточнения + гейты), но Proof запустить нельзя до
   Фазы 6 (`research_enabled=false`). Подтвердить, что это желаемый
   промежуточный опыт.
4. **Copy** всех новых пользовательских текстов (§4) — предложу в
   реализации, финал за владельцем (как в Фазе 3).
