...
Статус: **утверждён владельцем 2026-08-20** (включая п.5: вариант A — `CANCEL_AT_PERIOD_END` сохраняет ARI • CORE entitlement до `valid_until`; отменяется только следующее продление; немедленное прекращение — отдельный сценарий refund/revocation).
