...
| `tests/adversarial-core-round12-degradation-v1.test.ts` | Round 12 — TECHNICAL DEGRADATION AND TEMPORAL MONOTONICITY, the pure half. Round 6 removed ONE thing at a time; this round builds a LATTICE: six independent technical degradations (supply read off, burn read off, current state aged out, destination route withdrawn, extraction not completed, a documentary component unreachable) give 64 worlds ordered by subset inclusion, and every covering pair is asserted for every intent — verdict never rises, the band never rises at an equal verdict, no world is NOT_SUPPORTED unless the control was, provenance holds everywhere, and the lattice is proven non-vacuous (each axis bites, the ruined world really is weaker). Then partial chain visibility (both readings / either alone / neither, and a reading that lost its binding), temporal pathologies (future-dated, undated, input order vs temporal order, stale support meeting fresh contradiction, historical execution vs current state), and confidence neutrality of the four absence shapes and of inadmissible rows at both lattice extremes. |
| `tests/adversarial-core-round12-degradation-db-v1.test.ts` | Round 12 — the persisted half: the same question over the same documents through the real worker and S4 executor with providers that fail the way providers fail (fetch 503, page gone, extractor returns nothing). Each degraded run is the FIRST research of its own fresh project, because a project whose pages were already acquired reuses those documents and the degradation would not bite. Pairs assert degraded <= healthy on verdict and band, no component becomes CONTRADICTED from absence, bounded research still finalises, total provider failure establishes and refutes nothing, the same failing environment is deterministic, and one project's bad day does not touch another's. |
| `tests/adversarial-core-round13-identity-v1.test.ts` | Round 13 — IDENTITY, PROVENANCE AND ISOLATION UNDER COMPOSITION, the pure half. Rounds 4 and 5.5 asked whether ONE foreign row binds; this round asks whether SEVERAL can complete a mechanism between them. A the jigsaw: revenue here + another project's mechanism + a third's execution; an unbound burn + an unbound supply delta; holders here + the entitlement bridge there; the full jigsaw with every missing component supplied by a different foreign identity; volume and permutation. The law asserted is byte identity with the isolated control, not merely "no stronger". B a foreign contradiction does not refute either, and a foreign row is RECORDED as refused (`WRONG_PROJECT`) without ever becoming a blocking gap. C provenance closure across every mixed world: each non-unsatisfied requirement cites only this job's admitted rows, names its components, and cites nothing excluded. |\n| `tests/adversarial-core-round13-identity-db-v1.test.ts` | Round 13 — the persisted half, where binding is actually computed: two onboarded projects with their own confirmed token identities and classified routes, and a provider that serves BOTH projects' pages, offering the foreign one first at every step. A the cross-served research is exactly the research the project would have done alone, and not one foreign-host row is written as its Evidence (the attack surface is asserted reached, and the comparison asserted non-vacuous). B Memory may carry an observation and never a verdict, and never across an identity: Project A's VERIFIED, ACTIVE memory is not adoptable by Project B, and A's Proof rewritten to SUPPORTED at confidence 99 moves nothing in B. C whole-table closure after every attack. |\n
